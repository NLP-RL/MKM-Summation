...
TARGET_COLUMN_1 = "Patient_Summary" #change
...
class MultimodalBartForConditionalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.overall_summary1 = nn.Linear(2*config.d_model, config.d_model, bias = False)
    ...
    def forward(
        ...
        overall1 = self.overall_summary1(torch.cat([patient1, doctor1], dim = -1))
        ...
        masked_lm_loss = 0
        masked_lm_loss = 0.1*loss_patient + 0.1*loss_doctor + 0.8*loss_overall
    ...
    # -> Union[BeamSearchOutput, torch.LongTensor] :
        ...
        # }
            ...
            outputs, patient, doctor, overall = self(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
...
def val_epoch(model,
    ...
    with torch.no_grad():
        for step, batch in enumerate(tqdm(data_loader, desc="Validation Loss Iteration")):
            ...
            input_ids, attention_mask, visual_input, kg_input, labels_1, labels_2, labels_3 = batch #change
            visual_input = visual_input.unsqueeze(dim = 1)
            kg_input = kg_input.unsqueeze(dim = 1)
            outputs_1, logits_1, logits_2, logits_3 = model(input_ids=input_ids,
                                                      attention_mask=attention_mask,
...
def train(model,
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        val_results_1, val_results_2, val_results_3= get_val_scores(model, #change
                                     tokenizer,
                                     val_data_loader,
        ...
        test_results_1, test_results_2, test_results_3 = get_val_scores(model, #change
                                      tokenizer,
                                      test_data_loader,
...
def get_val_scores(model,
                   ...
                   epoch,
                   **gen_kwargs):
    predictions_1, gold_1, predictions_2, gold_2, predictions_3, gold_3 = test_epoch(model, #change
                                   tokenizer,
                                   data_loader,
                                   desc=desc,
                                   **gen_kwargs)
    ...
    if "Test" in desc:
        ...
        test_df = pd.DataFrame(list(zip(gold_1, predictions_1, gold_2, predictions_2, gold_3, predictions_3)), columns=['actual_1', 'predicted_1', 'actual_2', 'predicted_2', 'actual_3', 'predicted_3'])
        file_name = "MTMDS/Ablation_5/pred_file.csv"
...
def test_epoch(model,
    ...
    with torch.no_grad():
        for step, batch in enumerate(tqdm(data_loader, desc=desc)):
            ...
            outputs_1, logits_1, logits_2, logits_3 = model(input_ids=input_ids, #change
                            attention_mask=attention_mask,
                            visual_input=visual_input,
                            kg_input=kg_input,
                            labels=labels_1,
    ...
    return predictions1, gold_1, predictions2, gold_2, predictions3, gold_3 #change
...
def train_epoch(model,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc="Training Iteration")):
        ...
        input_ids, attention_mask, visual_input, kg_input, labels_1, labels_2, labels_3 = batch
        visual_input = visual_input.unsqueeze(dim = 1)
        kg_input = kg_input.unsqueeze(dim = 1)
        # print("-----------------------------------labels_1----------------------------------------")
        # print(labels_1)
...
test_data.to_csv("MTMDS/Ablation_5/test_data.csv")
# print(type(train_data["concated_transcript"].values))
# print(1/0)
train_data.to_json("MTMDS/T5/train.json")
valid_data.to_json("MTMDS/T5/valid.json")
test_data.to_json("MTMDS/T5/test.json")
